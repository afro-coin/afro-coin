...
class MempoolLimitTest(AfrocoinTestFramework):
    ...
    def test_mid_package_eviction_success(self):
        ...
        fees_afc_per_kvb = [entry["fees"]["base"] / (Decimal(entry["vsize"]) / 1000) for entry in mempool_entries]
        mempool_entry_minrate = min(fees_afc_per_kvb)
        ...
        for eviction in eviction_set:
            ...
            for txid, entry in zip(mempool_txids, mempool_entries):
                if txid == eviction:
                    evicted_feerate_afc_per_kvb = entry["fees"]["modified"] / (Decimal(entry["vsize"]) / 1000)
                    assert_greater_than(evicted_feerate_afc_per_kvb, max_parent_feerate)
    ...
    def test_mid_package_eviction(self):
        ...
        # Specific number of cents to fit within a small window. The parent_cpfp + child package needs to be
        ...
        magic_cents = 1200
        cpfp_cents = int(cpfp_fee * COIN) + magic_cents

        child = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos, fee_per_output=cpfp_cents)
        package_hex.append(child["hex"])
    ...
    def run_test(self):
        ...
        worst_feerate_afcvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_afcvb = min(worst_feerate_afcvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_feerate = (worst_feerate_afcvb * 1000) - Decimal("0.000001")  # 0.1 sats/vbyte below worst feerate
        ...
        assert_greater_than(worst_feerate_afcvb, package_fee / package_vsize)
