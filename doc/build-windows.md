...
Below are some notes on how to build Afrocoin Core for Windows.

The options known to work for building Afrocoin Core on Windows are:
...
Note that for WSL the Afrocoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/afrocoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\afrocoin`, for example:

    cmake --install build --prefix /mnt/c/workspace/afrocoin
