# CJDNS support in Afrocoin Core

It is possible to run Afrocoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Afrocoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Afrocoin network and individual nodes.
...
before it will work with your Afrocoin Core node. This is described in steps
...
## Run Afrocoin Core with CJDNS

Once you are connected to the CJDNS network, the following Afrocoin Core
...
When enabled, this option tells Afrocoin Core that it is running in an
...
IPv6 local network. This helps Afrocoin Core perform better address management:
...
CJDNS support was added to Afrocoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `afrocoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/afrocoin/afrocoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Afrocoin Core

There are several ways to see your CJDNS address in Afrocoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `afrocoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `afrocoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `afrocoin-cli help getnodeaddresses` for details.
