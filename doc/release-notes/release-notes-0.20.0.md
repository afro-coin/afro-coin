...
Afrocoin Core version 0.20.0 is now available from:

  <https://afrocoincore.org/bin/afrocoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Afrocoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Afrocoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Afrocoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Afrocoin Core. Afrocoin Core logs debug messages
...
- OpenSSL is no longer used by Afrocoin Core.  (#17265)

- BIP70 support has been fully removed from Afrocoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/afrocoin/afrocoin/issues/16599),
  [PR #16702](https://github.com/afrocoin/afrocoin/pull/16702), and the
  `afrocoind help` for more information.  This option is experimental and
...
- All custom settings configured when Afrocoin Core starts are now
...
- The "Start Afrocoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://afrocoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Afrocoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Afrocoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/afrocoin-core/HWI) for
  signing.  Future versions of Afrocoin Core should support a GUI option
...
- Afrocoin Core's automatically-generated source code documentation is
  now available at https://doxygen.afrocoincore.org.  (#17596)
...
- The `afrocoin-cli` utility used with the `-getinfo` parameter now
...
- The `afrocoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `afrocoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_afrocoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_afrocoin` binary (hebasto)
...
- #18702 Fix ASLR for afrocoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate afrocoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest afrocoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_afrocoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (afrocoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone afrocoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Afrocoin Core files (hebasto)
...
- #17119 Fix broken afrocoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from afrocoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.afrocoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to afrocoin-wallet (hel-o)
...
- #18341 Replace remaining literal AFC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On afrocoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from afrocoin-cli -getinfo (malevolent)
