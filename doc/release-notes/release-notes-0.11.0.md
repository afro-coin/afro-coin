Afrocoin Core version 0.11.0 is now available from:

  <https://afrocoin.org/bin/afrocoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer AFC/kB fee to be
...
For example, add the following to `afrocoin.conf`:
...
related to the blockchain in the afrocoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Afrocoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Afrocoin Core will 
...
For an example script see [afrocoin-submittx](https://github.com/laanwj/afrocoin-submittx).
...
- #5334 `cf87536` libafrocoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new afrocoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only AfrocoinAmountField
- #5753 `0f44672` Add afrocoin logo to about screen
...
- #5809 `46bfbe7` Add afrocoin-cli man page
...
- #5849 `41259ca` contrib/init/afrocoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Afrocoin-Qt.app to Afrocoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for afrocoin-tx)
