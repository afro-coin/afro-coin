...
This guide describes how to build afrocoind, command-line utilities, and GUI on FreeBSD.
...
Now that `git` and all the required dependencies are installed, let's clone the Afrocoin Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `afrocoind` or `afrocoin-qt`.
...
```
to: /path/to/afrocoin/depends/x86_64-unknown-freebsd[release-number]
```
...
Afrocoin Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
