# PSBT Howto for Afrocoin Core

Since Afrocoin Core 0.17, an RPC interface exists for Partially Signed Afrocoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/afrocoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Afrocoin transactions that are not fully signed
...
#### Multisig with multiple Afrocoin Core instances
...
Afrocoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `afrocoin-cli` is used.
...
Later, when *V* AFC has been received on *Amulti*, and Bob and Carol want to
