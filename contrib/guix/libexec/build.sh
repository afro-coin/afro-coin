#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Afrocoin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/afrocoin-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Afrocoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Afrocoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    case "$HOST" in
        *darwin*)
            ...
            mv build/dist/Afrocoin-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
    ...
    (
        ...
        # copy over the example afrocoin.conf file. if contrib/devtools/gen-afrocoin-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/afrocoin.conf" "${DISTNAME}/"
