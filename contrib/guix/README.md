# Bootstrappable Afrocoin Core Builds

This directory contains the files necessary to perform bootstrappable Afrocoin
Core builds.
...
To build Afrocoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Afrocoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/afrocoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/afrocoin-core/guix.sigs).
