...
"""Script for verifying Afrocoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from afrocoincore.org and afrocoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/afrocoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://afrocoincore.org"
HOST2 = "https://afrocoin.org"
VERSIONPREFIX = "afrocoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Afrocoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"afrocoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by afrocoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the afrocoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes afrocoin.org lags behind afrocoincore.org.)')
    )
