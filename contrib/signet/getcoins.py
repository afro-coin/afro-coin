#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Afrocoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing afrocoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='afrocoin-cli', help='afrocoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Afrocoin address to which the faucet should send')
...
parser.add_argument('afrocoin_cli_args', nargs='*', help='Arguments to pass on to afrocoin-cli (default: -signet)')

args = parser.parse_args()

if args.afrocoin_cli_args == []:
    args.afrocoin_cli_args = ['-signet']


def afrocoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.afrocoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = afrocoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = afrocoin_cli(['getnewaddress', 'faucet', 'bech32'])
