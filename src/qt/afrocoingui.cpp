// Copyright (c) 2011-2022 The Afrocoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <afrocoin-build-config.h> // IWYU pragma: keep

#include <qt/afrocoingui.h>
...
const std::string AfrocoinGUI::DEFAULT_UIPLATFORM =
...
AfrocoinGUI::AfrocoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &AfrocoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &AfrocoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &AfrocoinGUI::showModalOverlay);
...
void AfrocoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Afrocoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and afrocoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &AfrocoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &AfrocoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &AfrocoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Afrocoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Afrocoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Afrocoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Afrocoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a afrocoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Afrocoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &AfrocoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &AfrocoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &AfrocoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &AfrocoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &AfrocoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &AfrocoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &AfrocoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &AfrocoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &AfrocoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &AfrocoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &AfrocoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &AfrocoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &AfrocoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &AfrocoinGUI::showDebugWindow);
}

void AfrocoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void AfrocoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &AfrocoinGUI::setCurrentWalletBySelectorIndex);
...
void AfrocoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &AfrocoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &AfrocoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &AfrocoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &AfrocoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void AfrocoinGUI::enableHistoryAction(bool privacy)
{
...
void AfrocoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &AfrocoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &AfrocoinGUI::removeWallet);
...
WalletController* AfrocoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void AfrocoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &AfrocoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &AfrocoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &AfrocoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &AfrocoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &AfrocoinGUI::incomingTransaction);
    connect(this, &AfrocoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void AfrocoinGUI::removeWallet(WalletModel* walletModel)
{
...
void AfrocoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void AfrocoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void AfrocoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AfrocoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void AfrocoinGUI::createTrayIcon()
{
...
void AfrocoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &AfrocoinGUI::toggleHidden);
...
void AfrocoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void AfrocoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AfrocoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void AfrocoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void AfrocoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void AfrocoinGUI::openClicked()
{
...
void AfrocoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AfrocoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AfrocoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AfrocoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AfrocoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AfrocoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void AfrocoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void AfrocoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Afrocoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void AfrocoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AfrocoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void AfrocoinGUI::updateHeadersSyncProgressLabel()
{
...
void AfrocoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void AfrocoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &AfrocoinGUI::quitRequested);
...
void AfrocoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void AfrocoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &AfrocoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void AfrocoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void AfrocoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &AfrocoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &AfrocoinGUI::show);
                e->ignore();
            }
        }
    }
...
void AfrocoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AfrocoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AfrocoinGUI::incomingTransaction(const QString& date, AfrocoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AfrocoinUnits::formatWithUnit(unit, amount, true));
...
void AfrocoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AfrocoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AfrocoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AfrocoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AfrocoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void AfrocoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AfrocoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void AfrocoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void AfrocoinGUI::updateWindowTitle()
{
...
void AfrocoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AfrocoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AfrocoinGUI::detectShutdown()
{
...
void AfrocoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void AfrocoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AfrocoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = AfrocoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void AfrocoinGUI::subscribeToCoreSignals()
{
...
void AfrocoinGUI::unsubscribeFromCoreSignals()
{
...
bool AfrocoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<AfrocoinUnit> units = AfrocoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AfrocoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, AfrocoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const AfrocoinUnit u : AfrocoinUnits::availableUnits()) {
        menu->addAction(AfrocoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(AfrocoinUnit newUnits)
{
    setText(AfrocoinUnits::longName(newUnits));
}
