...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<AfrocoinUnit>("AfrocoinUnit");
#else
    qRegisterMetaType<AfrocoinUnit>("AfrocoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. afrocoin_de.qm (shortcut "de" needs to be defined in afrocoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. afrocoin_de_DE.qm (shortcut "de_DE" needs to be defined in afrocoin.qrc)
...
static const char* qt_argv = "afrocoin-qt";

AfrocoinApplication::AfrocoinApplication()
...
void AfrocoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the AfrocoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", AfrocoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void AfrocoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool AfrocoinApplication::createOptionsModel(bool resetSettings)
{
...
void AfrocoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new AfrocoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &AfrocoinGUI::quitRequested, this, &AfrocoinApplication::requestShutdown);
...
void AfrocoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void AfrocoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool AfrocoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void AfrocoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &AfrocoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &AfrocoinApplication::handleRunawayException);
    connect(this, &AfrocoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &AfrocoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void AfrocoinApplication::parameterSetup()
{
...
void AfrocoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void AfrocoinApplication::requestInitialize()
{
...
void AfrocoinApplication::requestShutdown()
{
...
void AfrocoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // afrocoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AfrocoinGUI::handlePaymentRequest);
            connect(window, &AfrocoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void AfrocoinApplication::handleRunawayException(const QString &message)
{
...
void AfrocoinApplication::handleNonFatalException(const QString& message)
{
...
WId AfrocoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool AfrocoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AfrocoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(afrocoin);
    Q_INIT_RESOURCE(afrocoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see afrocoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see afrocoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse afrocoin.conf, determine network, switch to network specific
    ...
    // afrocoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
