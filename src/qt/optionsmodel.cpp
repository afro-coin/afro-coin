...
/** Call node.updateRwSetting() with Afrocoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Afrocoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/afrocoin/afrocoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because afrocoin 22.x will not try to read these.
...
//! Convert enabled/size values to afrocoin -prune setting.
...
//! Get pruning enabled value to show in GUI from afrocoin -prune setting.
...
//! Get pruning size value to show in GUI from afrocoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayAfrocoinUnit")) {
        settings.setValue("DisplayAfrocoinUnit", QVariant::fromValue(AfrocoinUnit::AFC));
    }
    QVariant unit = settings.value("DisplayAfrocoinUnit");
    if (unit.canConvert<AfrocoinUnit>()) {
        m_display_afrocoin_unit = unit.value<AfrocoinUnit>();
    } else {
        m_display_afrocoin_unit = AfrocoinUnit::AFC;
        settings.setValue("DisplayAfrocoinUnit", QVariant::fromValue(m_display_afrocoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if afrocoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_afrocoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<AfrocoinUnit>() == m_display_afrocoin_unit) return;
    m_display_afrocoin_unit = new_unit.value<AfrocoinUnit>();
    QSettings settings;
    settings.setValue("DisplayAfrocoinUnit", QVariant::fromValue(m_display_afrocoin_unit));
    Q_EMIT displayUnitChanged(m_display_afrocoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/afrocoin/afrocoin/pull/8273
    ...
    // (https://github.com/afrocoin-core/gui/issues/567).
