...
{
    GUIUtil::setClipboard(AfrocoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of cents so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    AfrocoinUnit nDisplayUnit = AfrocoinUnit::AFC;
    ...
    l2->setText(AfrocoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(AfrocoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(AfrocoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(AfrocoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    // how many cents the estimated fee can vary per byte we guess wrong
...
void CoinControlDialog::updateView()
{
    ...
    AfrocoinUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show afrocoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, AfrocoinUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, AfrocoinUnits::format(nDisplayUnit, nSum));
