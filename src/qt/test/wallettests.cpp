                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<AfrocoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = AfrocoinUnits::formatWithUnit(unit, expected_balance, false, AfrocoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_afrocoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_afrocoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_afrocoin-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    AfrocoinAmountField* amountInput = receiveCoinsDialog.findChild<AfrocoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_afrocoin-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
