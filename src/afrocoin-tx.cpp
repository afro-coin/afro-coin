...
static void SetupAfrocoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = CLIENT_NAME " afrocoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The afrocoin-tx tool is used for creating and modifying afrocoin transactions.\n\n"
                "afrocoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded afrocoin transaction, or with \"-create [commands]\" to create a hex-encoded afrocoin transaction.\n"
                "\n"
                "Usage: afrocoin-tx [options] <hex-tx> [commands]\n"
                "or:    afrocoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded afrocoin transaction
