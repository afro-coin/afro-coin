...
const char * const BITCOIN_CONF_FILENAME = "afrocoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //afrocoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Afrocoin
    //   new: C:\Users\Username\AppData\Local\Afrocoin
    // macOS: ~/Library/Application Support/Afrocoin
    // Unix-like: ~/.afrocoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Afrocoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Afrocoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Afrocoin";
#else
    // Unix-like
    return pathRet / ".afrocoin";
#endif
#endif
}
